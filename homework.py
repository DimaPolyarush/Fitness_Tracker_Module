...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        return ((self.WALK_COEFF_CALORIE_1 * self.weight
...
class Swimming(Training):
    ...
    def get_spent_calories(self) -> float:
        return ((self.get_mean_speed() + self.SWIM_COEFF_CALORIE_1)
                * self.SWIM_COEFF_CALORIE_2 * self.weight)
...
def read_package(workout_type: str, data: list) -> Training:
    ...
    if workout_type not in type_traing:
        raise ValueError('Неожиданный тип тренировки')
    return type_traing[workout_type](*data)
